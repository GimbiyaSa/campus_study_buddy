...
module.exports = async function (context, myTimer) {
  ...
  try {
    ...
    const { resources: upcomingSessions } = await sessionsContainer.items
      .query(querySpec)
      .fetchAll();
    ...
    for (const session of upcomingSessions) {
      ...
      const { resource: group } = await groupsContainer
        .item(session.groupId, session.partitionKey.split('T')[0])
        .read();

      for (const member of group.members) {
        // Get user email
        const { resource: user } = await usersContainer
          .item(member.userId, group.partitionKey)
          .read();
