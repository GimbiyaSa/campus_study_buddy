...
describe('Course Service API', () => {
  ...
  describe('GET /courses', () => {
    test('returns courses list with pagination and default parameters', async () => {
      mockQuery.mockResolvedValueOnce({ recordset: [
        { 
          id: 1, code: 'CS101', title: 'CS101', description: 'Intro to Computer Science', 
          university: 'Test University', status: 'active', enrolled_at: new Date('2025-01-01'), 
          createdAt: new Date('2025-01-01'), updatedAt: new Date('2025-01-01'), 
          progress: 50, totalHours: 10, totalTopics: 5, completedTopics: 2, 
          weeklyStudyDays: 2, lastStudiedAt: new Date('2025-01-01') 
        }
      ] });
    ...
    test('transforms course data correctly for frontend', async () => {
      const mockCourse = {
        id: 1, code: 'CS101', title: 'Computer Science', description: 'Intro',
        university: 'Test University', status: 'active', 
        progress: 75.5, totalHours: 12.7, totalTopics: 8, completedTopics: 6,
        weeklyStudyDays: 3, lastStudiedAt: new Date('2025-01-01'),
        createdAt: new Date('2025-01-01'), updatedAt: new Date('2025-01-01')
      };
  ...
  describe('POST /courses', () => {
    ...
    test('validates title field type', async () => {
      const res = await request(app)
        .post('/courses')
        .send({ type: 'institution', title: 123 });
      expect(res.statusCode).toBe(400);
      expect(res.body.error).toContain('Title is required');
    });
    ...
    test('handles institutional enrollment with valid moduleId', async () => {
      ...
      mockQuery.mockResolvedValueOnce({ 
        recordset: [{ 
          module_id: 1, module_code: 'CS101', module_name: 'Computer Science',
          description: 'Intro', university: 'Test University' 
        }] 
      });
  ...
  describe('PUT /courses/:id', () => {
    ...
    test('handles valid status update', async () => {
      ...
      mockQuery.mockResolvedValueOnce({ recordset: [{ 
        user_module_id: 1, module_code: 'CS101', module_name: 'Computer Science',
        description: 'Intro', university: 'Test University' 
      }] });
      // Mock update query
      mockQuery.mockResolvedValueOnce({ recordset: [] });
      ...
      mockQuery.mockResolvedValueOnce({ recordset: [{
        id: 1, code: 'CS101', title: 'Computer Science', description: 'Intro',
        university: 'Test University', status: 'active', progress: 50,
        totalHours: 10, createdAt: new Date(), updatedAt: new Date()
      }] });
  ...
  describe('DELETE /courses/:id', () => {
    test('successfully deletes existing enrollment', async () => {
      ...
      mockQuery.mockResolvedValueOnce({ 
        recordset: [{ 
          user_module_id: 1, module_name: 'CS101', 
          module_code: 'CS101', university: 'Test University' 
        }] 
      });
  ...
  describe('GET /courses/available', () => {
    test('returns available courses successfully', async () => {
      mockQuery.mockResolvedValueOnce({ recordset: [
        { 
          id: 1, code: 'CS101', title: 'CS101', description: 'Intro', 
          university: 'Test University', isEnrolled: 0, enrolledCount: 10,
          createdAt: new Date('2025-01-01') 
        },
        { 
          id: 2, code: 'CS102', title: 'CS102', description: 'Data Structures', 
          university: 'Test University', isEnrolled: 1, enrolledCount: 12,
          createdAt: new Date('2025-01-01') 
        }
      ] });
  ...
  describe('GET /courses/:id/topics', () => {
    test('returns topics for enrolled course', async () => {
      ...
      mockQuery.mockResolvedValueOnce({ recordset: [
        { 
          id: 1, name: 'Topic 1', description: 'Desc', order_sequence: 1, 
          completionStatus: 'completed', hoursSpent: 2, startedAt: new Date(), 
          completedAt: new Date(), chapterCount: 2, completedChapters: 2 
        },
        { 
          id: 2, name: 'Topic 2', description: 'Desc', order_sequence: 2, 
          completionStatus: 'not_started', hoursSpent: 0, startedAt: null, 
          completedAt: null, chapterCount: 3, completedChapters: 0 
        }
      ] });
  ...
  describe('GET /courses/:id/details', () => {
    test('returns course details successfully', async () => {
      ...
      mockQuery.mockResolvedValueOnce({ recordset: [{ 
        id: 1, code: 'CS101', title: 'CS101', description: 'Intro', 
        university: 'Test University', status: 'active', enrolledAt: new Date() 
      }] });
      // Mock topics
      mockQuery.mockResolvedValueOnce({ recordset: [
        { 
          id: 1, name: 'Topic 1', description: 'Desc', orderSequence: 1, 
          estimatedHours: 2, completionStatus: 'completed', hoursSpent: 2, 
          totalHours: 2, startedAt: new Date(), completedAt: new Date(), notes: '' 
        }
      ] });
  ...
  describe('POST /courses/:id/log-hours', () => {
    ...
    test('handles valid log-hours request', async () => {
      ...
      mockQuery.mockResolvedValueOnce({ recordset: [{ 
        hour_id: 1, hours_logged: 2.5, description: 'Study session',
        study_date: new Date(), logged_at: new Date()
      }] });
  ...
  describe('GET /courses/debug', () => {
    test('returns debug information successfully', async () => {
      mockQuery.mockResolvedValueOnce({ recordset: [
        { 
          module_id: 1, module_code: 'CS101', module_name: 'CS101', 
          description: 'Intro', university: 'Test University', 
          is_active: 1, enrollment_status: 'active', enrolled_at: new Date() 
        }
      ] });
  ...
  describe('Additional POST /courses Coverage', () => {
    ...
    test('handles duplicate course detection', async () => {
      ...
      mockQuery.mockResolvedValueOnce({ recordset: [{ 
        module_name: 'CS101', module_code: 'CS101', university: 'Test University' 
      }] });
  ...
  describe('Advanced Error Handling', () => {
    ...
    test('covers complex search and pagination logic', async () => {
      ...
      const res = await request(app).get('/courses?search=computer&page=5&limit=10&sortBy=progress&sortOrder=DESC');
      expect(res.statusCode).toBe(200);
  ...
  describe('Edge Cases and Data Transformation', () => {
    test('handles Custom university course type transformation', async () => {
      const mockCourse = {
        id: 1, code: 'CASUAL_123', title: 'Custom Course', description: 'Custom study',
        university: 'Custom', status: 'active', 
        progress: 25.7, totalHours: 5.23, totalTopics: 3, completedTopics: 1,
        weeklyStudyDays: 1, lastStudiedAt: null,
        createdAt: new Date('2025-01-01'), updatedAt: new Date('2025-01-01')
      };
      mockQuery.mockResolvedValueOnce({ recordset: [mockCourse] });
      mockQuery.mockResolvedValueOnce({ recordset: [{ total: 1 }] });
    ...
    test('handles null lastStudiedAt in data transformation', async () => {
      const mockCourse = {
        id: 1, code: 'CS101', title: 'Computer Science', description: 'Intro',
        university: 'Test University', status: 'active', 
        progress: 0, totalHours: 0, totalTopics: 5, completedTopics: 0,
        weeklyStudyDays: 0, lastStudiedAt: null,
        createdAt: new Date('2025-01-01'), updatedAt: new Date('2025-01-01')
      };
      mockQuery.mockResolvedValueOnce({ recordset: [mockCourse] });
      mockQuery.mockResolvedValueOnce({ recordset: [{ total: 1 }] });
    ...
    test('validates PUT endpoint with all valid status values', async () => {
      for (const status of ['active', 'completed', 'dropped']) {
        ...
        mockQuery.mockResolvedValueOnce({ recordset: [{
          id: 1, code: 'CS101', title: 'Computer Science', description: 'Intro',
          university: 'Test University', status: status, progress: 50,
          totalHours: 10, createdAt: new Date(), updatedAt: new Date()
        }] });
    ...
    test('covers test-search endpoint functionality', async () => {
      mockQuery.mockResolvedValueOnce({ recordset: [
        { module_name: 'Computer Science', module_code: 'CS101' }
      ] });
