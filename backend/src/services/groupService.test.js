...
jest.mock('@azure/cosmos', () => {
  ...
  const fakeGroupsItems = {
    query: jest.fn().mockReturnValue({ fetchAll: jest.fn().mockResolvedValue({ resources: fakeGroups }) }),
    create: jest.fn().mockImplementation(async (g) => ({ resource: g })),
  };

  const fakeSessionsItems = { create: jest.fn().mockImplementation(async (s) => ({ resource: s })) };
  ...
  const fakeDatabase = () => ({
    ...
    containers: { createIfNotExists: jest.fn().mockResolvedValue({ container: fakeContainer('Groups') }) },
  });
...
describe('Group service', () => {
  test('POST /api/v1/groups creates a group', async () => {
    const res = await request(app).post('/api/v1/groups').send({ name: 'G', description: 'D', subjects: ['s'] });
  ...
  test('POST /api/v1/groups/:groupId/sessions returns 201 when member', async () => {
    const res = await request(app).post('/api/v1/groups/g1/sessions').send({ title: 'S', startTime: new Date().toISOString() });
    expect(res.statusCode).toBe(201);
