...
describe('Module Service API', () => {
  describe('GET /modules', () => {
    test('returns modules list with default parameters', async () => {
      mockQuery.mockResolvedValueOnce({ recordset: [
        { module_id: 1, module_code: 'CS101', module_name: 'Intro to Computer Science', university: 'Test University', is_active: 1, enrolled_count: 5, topic_count: 3 }
      ] });
  ...
  describe('GET /modules/:moduleId', () => {
    test('returns module details when found', async () => {
      mockQuery.mockResolvedValueOnce({ recordset: [
        { module_id: 1, module_code: 'CS101', module_name: 'Intro to Computer Science', university: 'Test University', is_active: 1, enrolled_count: 5, topic_count: 3, study_group_count: 2 }
      ] });
  ...
  describe('GET /modules/:moduleId/topics', () => {
    test('returns topics for a module', async () => {
      mockQuery.mockResolvedValueOnce({ recordset: [
        { topic_id: 1, module_id: 1, topic_name: 'Variables', description: 'Learn about variables', order_sequence: 1, chapter_count: 2 }
      ] });
  ...
  describe('GET /modules/topics/:topicId/chapters', () => {
    test('returns chapters for a topic', async () => {
      mockQuery.mockResolvedValueOnce({ recordset: [
        { chapter_id: 1, topic_id: 1, chapter_name: 'Introduction', description: 'Basic intro', order_sequence: 1 }
      ] });
  ...
  describe('POST /modules', () => {
    test('creates new module successfully', async () => {
      mockQuery
        ...
        .mockResolvedValueOnce({ recordset: [{ module_id: 1, module_code: 'CS101', module_name: 'New Module', university: 'Test University' }] }); // SELECT query
    ...
    test('creates module without description', async () => {
      mockQuery
        ...
        .mockResolvedValueOnce({ recordset: [{ module_id: 1, module_code: 'CS102', module_name: 'Module 2', university: 'Test University' }] });
    ...
    test('returns 400 when required fields are missing', async () => {
      ...
      expect(res.body).toHaveProperty('error', 'module_code, module_name, and university are required');
    });
  ...
  describe('POST /modules/:moduleId/topics', () => {
    test('creates new topic successfully', async () => {
      mockQuery
        .mockResolvedValueOnce({ recordset: [] })
        .mockResolvedValueOnce({ recordset: [{ topic_id: 1, module_id: 1, topic_name: 'New Topic', description: 'Topic description', order_sequence: 1 }] });
      ...
      expect(mockInput).toHaveBeenCalledWith('description', expect.any(Function), 'Topic description');
    ...
    test('creates topic with minimal data', async () => {
      mockQuery
        .mockResolvedValueOnce({ recordset: [] })
        .mockResolvedValueOnce({ recordset: [{ topic_id: 1, module_id: 1, topic_name: 'Minimal Topic' }] });
  ...
  describe('POST /modules/topics/:topicId/chapters', () => {
    test('creates new chapter successfully', async () => {
      mockQuery
        .mockResolvedValueOnce({ recordset: [] })
        .mockResolvedValueOnce({ recordset: [{ chapter_id: 1, topic_id: 1, chapter_name: 'New Chapter', description: 'Chapter description', order_sequence: 1, content_summary: 'Summary' }] });
      ...
      expect(mockInput).toHaveBeenCalledWith('description', expect.any(Function), 'Chapter description');
      expect(mockInput).toHaveBeenCalledWith('orderSequence', expect.any(Function), 1);
    ...
    test('creates chapter with minimal data', async () => {
      mockQuery
        .mockResolvedValueOnce({ recordset: [] })
        .mockResolvedValueOnce({ recordset: [{ chapter_id: 1, topic_id: 1, chapter_name: 'Minimal Chapter' }] });
  ...
  describe('PUT /modules/:moduleId', () => {
    test('updates module successfully', async () => {
      mockQuery
        ...
        .mockResolvedValueOnce({ recordset: [{ module_id: 1, module_name: 'Updated Module', description: 'Updated description' }] }); // SELECT query
      ...
      expect(mockInput).toHaveBeenCalledWith('description', expect.any(Function), 'Updated description');
    });
    ...
    test('updates only module_name', async () => {
      ...
      expect(res.statusCode).toBe(200);
      expect(mockInput).toHaveBeenCalledWith('module_name', expect.any(Function), 'Only Name Updated');
      expect(mockInput).not.toHaveBeenCalledWith('description', expect.anything(), expect.anything());
    });
