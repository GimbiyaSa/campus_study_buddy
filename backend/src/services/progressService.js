...
router.post('/sessions', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            studyHoursRequest.input('topicId', sql.Int, topicIds && topicIds.length > 0 ? topicIds[0] : null);
...
router.get('/analytics', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const analytics = {
            ...
            averageSessionLength: studyHours.length > 0 ? 
                studyHours.reduce((sum, sh) => sum + sh.hours_logged, 0) / studyHours.length : 0,
            
            // Topics and modules studied
            topicsStudied: [...new Set(studyHours.filter(sh => sh.topic_name).map(sh => sh.topic_name))],
            modulesStudied: [...new Set(studyHours.filter(sh => sh.module_name).map(sh => sh.module_name))],
...
router.get('/modules/:moduleId', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const topics = topicsResult.recordset.map(topic => ({
            ...
            progress: topic.total_chapters > 0 ? Math.round((topic.completed_chapters / topic.total_chapters) * 100) : 0
        }));
...
function generateDailyBreakdown(studyHours, days) {
    ...
    studyHours.forEach((entry) => {
        ...
        if (breakdown[dateKey]) {
            ...
            breakdown[dateKey].hours = Math.round((breakdown[dateKey].hours + entry.hours_logged) * 100) / 100;
        }
    });
...
function generateModuleBreakdown(studyHours) {
    ...
    studyHours.forEach((entry) => {
        ...
        breakdown[moduleKey].hours = Math.round((breakdown[moduleKey].hours + entry.hours_logged) * 100) / 100;
    });
