...
export default function ProgressTracking() {
  ...
  const loadProgressData = async () => {
    try {
      ...
      const [progressResponse, sessionsResponse, achievementsResponse] = await Promise.all([
                ...
                {weeklyChartData.datasets[0].data.map((hours, index) => (
                  <div key={index} className="flex flex-col items-center flex-1">
                    <div
                      ...
                      style={{
                        height: `${Math.max(
                          (hours / Math.max(...weeklyChartData.datasets[0].data)) * 100,
                          2
                        )}%`,
                      }}
                    />
                    <span className="text-xs text-gray-600 mt-2">
                      {weeklyChartData.labels[index]}
                    </span>
          ...
          {achievements.length === 0 ? (
            ...
            <div className="space-y-3">
              {achievements.slice(0, 4).map((achievement) => (
                <div
                  key={achievement.id}
                  className="flex items-center gap-3 p-3 bg-gradient-to-r from-yellow-50 to-yellow-100 border border-yellow-200 rounded-lg"
                >
        ...
        {timeDistributionData ? (
          <div className="grid grid-cols-4 gap-4">
            {timeDistributionData.labels.map((label, index) => {
              ...
              const totalHours = timeDistributionData.datasets[0].data.reduce(
                (sum, h) => sum + h,
                0
              );
              const percentage = totalHours > 0 ? (hours / totalHours) * 100 : 0;
                ...
                <div key={label} className="text-center">
                  <div className="mb-3">
                    <div
                      ...
                      style={{
                        backgroundColor: timeDistributionData.datasets[0].backgroundColor?.[index],
                      }}
                    >
                      {hours.toFixed(1)}h
                    </div>
