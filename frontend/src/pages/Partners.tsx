    ...
    <div className="space-y-6">
      ...
      <section className={card} aria-labelledby="suggestions-title">
        ...
        <ul className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
          {suggestions.map((s, i) => (
            <li key={i} className="flex items-center justify-between gap-3 p-3 rounded-xl border border-gray-100 hover:bg-gray-50/60 transition">
      ...
      <section className={card} aria-labelledby="search-title">
        <div className="mb-4 flex items-center justify-between gap-3">
          <h2 id="search-title" className="font-semibold text-gray-900">Search & filter</h2>
          <button
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center">
          <label htmlFor="q" className="sr-only">Search by name, course, tag</label>
          ...
          <div className="flex items-center gap-2">
            ...
            <label htmlFor="mutual" className="text-sm text-gray-700">Min mutual courses:</label>
            <input
              id="mutual"
              type="number"
              min={0}
        ...
        <ul className="mt-3 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
          {results.map((p, i) => (
            <li key={`r-${i}`} className="flex items-center justify-between gap-3 p-3 rounded-xl border border-gray-100 hover:bg-gray-50/60 transition">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-brand-100 text-brand-700 grid place-items-center text-sm font-semibold">
                  {p.initials}
                </div>
                <div>
                  ...
                  <div className="mt-1 flex flex-wrap gap-1">
                    <span className="text-[11px] px-2 py-0.5 rounded-full bg-gray-100 text-gray-600">{p.overlap}</span>
                    {p.tags.map((t) => (
                      <span key={t} className="text-[11px] px-2 py-0.5 rounded-full bg-brand-50 text-brand-700">
                        {t}
                      </span>
