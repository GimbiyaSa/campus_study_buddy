...
test('can expand and collapse course cards', async () => {
  ...
  const expandButton = expandButtons.find(btn => 
    btn.querySelector('svg') && btn.getAttribute('class')?.includes('hover:bg-slate-100')
  );
...
test('handles course navigation links', async () => {
  ...
  const csLink = courseLinks.find(link => link.textContent?.includes('Intro to Computer Science'));
