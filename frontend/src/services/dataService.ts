...
export class DataService {
  ...
  static async fetchNotifications(opts?: {
    ...
    try {
      const res = await this.request(`/api/v1/notifications?${params.toString()}`, {
        headers: this.authHeaders(),
      });
      if (!res.ok) return [];
  ...
  static async markNotificationRead(notificationId: number): Promise<boolean> {
    try {
      const res = await this.request(`/api/v1/notifications/${notificationId}/read`, {
        method: 'PUT',
        headers: this.jsonHeaders(),
      });
      return res.ok;
  ...
  // Trigger 24h reminder scheduling for a single session (backend enqueues per attendee)
