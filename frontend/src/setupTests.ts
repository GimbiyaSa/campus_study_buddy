  ...
  global.fetch = vi.fn((input: string | URL | Request) => {
    ...
    if (url.includes('/api/v1/partners')) {
      ...
      return Promise.resolve({
        ok: true,
        json: () => Promise.resolve([
          { id: '2', name: 'Alice Smith', major: 'Mathematics', overlap: '2 modules', tags: ['Morning', 'On-campus'], initials: 'AS' },
          { id: '3', name: 'Bob Lee', major: 'Physics', overlap: '1 module', tags: ['Evening'], initials: 'BL' },
        ]),
      }) as Promise<Response>;
    }
    ...
    if (url.includes('/api/v1/courses')) {
      return Promise.resolve({
        ok: true,
        json: () => Promise.resolve([
          { id: 'CS101', title: 'Intro to Computer Science', code: 'CS101', progress: 80, totalHours: 40, weeklyHours: 5, lastStudied: '2025-09-20' },
          { id: 'MATH201', title: 'Calculus II', code: 'MATH201', progress: 60, totalHours: 30, weeklyHours: 3, lastStudied: '2025-09-18' },
        ]),
      }) as Promise<Response>;
    }
    ...
    if (url.includes('/api/v1/user/course-progress')) {
      return Promise.resolve({
        ok: true,
        json: () => Promise.resolve([
          { id: 'CS101', title: 'Intro to Computer Science', progress: 80, totalHours: 40, weeklyHours: 5, lastStudied: '2025-09-20' },
        ]),
      }) as Promise<Response>;
    }
